...
def draw_boxes(detections, image, colors, y_axis):
    import cv2
    for det in detections:
        left, top, right, bottom = int(det[0]),int(det[1]),int(det[2]),int(det[3])
        id = int(det[6])
        size = (det[2] - det[0]) * (det[3] - det[1])
        label = ''
        if y_axis <= bottom:
            if det[5] < size and det[4] < y_axis:
                label = 'coming'
            else:
                label = 'warning'
        cv2.rectangle(image, (left, top), (right, bottom), colors['bird'], 2)
        cv2.putText(image, "{} {}".format(id,label),
